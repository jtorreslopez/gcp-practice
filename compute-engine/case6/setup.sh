...
gcloud compute firewall-rules create network-1-allow-ssh-icmp --network=network-1 --direction=INGRESS --action=ALLOW --rules=tcp:22,icmp --target-tags=client,http-server
gcloud compute firewall-rules create network-1-allow-http --network=network-1 --direction=INGRESS --action=ALLOW --rules=tcp:80 --target-tags=http-server --source-ranges=10.50.0.0/26
...
gcloud compute instance-groups managed create backend-2-group --zone=us-west1-c --template=backend-2-template --base-instance-name=backend-2 --size=2
...
gcloud compute backend-services add-backend backend-service-2 --instance-group=backend-2-group --instance-group-zone=us-west1-c --region=us-west1
...
# '''''''''''''''''''''''''''''''''''''''''''''
gcloud compute instances create client --zone=us-west1-b --tags=client --machine-type=f1-micro --boot-disk-size=20GB --network=network-1 --subnet=network-1-subnet-clients
